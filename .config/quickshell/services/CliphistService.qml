...
QtObject {
    ...
    function copyEntry(entry) {
        Quickshell.execDetached(["bash", "-c", "printf '" + shellEscape(entry) + "' | cliphist decode | wl-copy"]);
    }

    function deleteEntry(entry) {
        Quickshell.execDetached(["bash", "-c", "printf '" + shellEscape(entry) + "' | cliphist delete"]);
    ...
    property var readProc: Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                root.entries = arr;
                // After loading entries, decode image previews
                root.refreshImages();
            }
        }
    }
    ...
    property var imgProc: Process {
        id: imgProc
        command: ["python3", "/home/sadid/.config/quickshell/scripts/decode_clip_images.py"]
